...
class TestInfections:
    ...
    def test_list_infections(self, api_client, sample_infections):
        ...
        assert len(data) == len(sample_infections), (
            f'Ожидалось {len(sample_infections)} инфекций, получено {len(data)}. Данные: {data}'
        )
    ...
    def test_sort_by_name_default_direction(self, api_client, sample_infections):
    ...
    def test_filter_by_category_name(self, api_client, sample_infections):
        ...
        for item in data:
            assert item['category'] == category.name, (
                f"Ожидался name категории '{category.name}', получен '{item['category']}'. Данные элемента: {item}"
            )

    def test_combined_sort_and_filter_by_name(self, api_client, sample_infections):
        """Комбинация сортировки и фильтрации по name категории."""
        category = sample_infections[0].category
        url = reverse(self.URL_LIST)
        params = {'category': category.name, 'sort_by': 'name', 'direction': 'asc'}
        response = api_client.get(f'{url}?{urlencode(params)}')
    ...
    def test_combined_sort_desc_and_filter_by_name(self, api_client, sample_infections):
        """Фильтрация по name с сортировкой по убыванию."""
        category = sample_infections[0].category
        url = reverse(self.URL_LIST)
        params = {'category': category.name, 'sort_by': 'name', 'direction': 'desc'}
        response = api_client.get(f'{url}?{urlencode(params)}')
        ...
        for item in data:
            assert item['category'] == category.name, (
                f"Фильтрация нарушена. Ожидался name '{category.name}', получен '{item['category']}'. Данные: {item}"
            )
        names = [item['name'] for item in data]
        expected = sorted(names, reverse=True)
        assert names == expected, (
            f'Сортировка по убыванию нарушена. Ожидалось: {expected}, получено: {names}. params: {params}'
        )
    ...
    def test_filter_by_category_empty_name(self, api_client, sample_infections):
        ...
        data = response.json()
        assert len(data) >= 0, f'Некорректный ответ для пустого name. Данные: {data}'
    ...
    def test_sort_without_sort_by(self, api_client, sample_infections):
        ...
        data = response.json()
        assert len(data) == len(sample_infections), (
            f'Ожидалось {len(sample_infections)} инфекций (все записи), получено {len(data)}. Данные: {data}'
        )
