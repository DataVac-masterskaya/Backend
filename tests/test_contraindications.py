...
class TestContraindications:
    ...
    def test_list_contraindications(self, api_client,
                                    sample_contraindications):
        ...
        assert len(data) == len(sample_contraindications), (
            f"Ожидалось {len(sample_contraindications)} противопоказаний, "
            f"получено {len(data)}. Данные: {data}"
        )
        assert data[0]['name'] == 'Аллергия на компоненты', (
            f"Ожидалось 'Аллергия на компоненты', "
            f"получено '{data[0]['name']}'. Данные: {data[0]}"
        )
    ...
    def test_list_categories(self, api_client, sample_categories):
        ...
        data = response.json()
        assert len(data) >= 2, (
            f"Ожидалось минимум 2 категории, получено {len(data)}. "
            f"Данные: {data}"
        )

        category_names = [item['name'] for item in data]
        assert 'Абсолютные' in category_names, (
            f"Категория 'Абсолютные' не найдена в ответе. "
            f"Доступные категории: {category_names}"
        )
        assert 'Относительные' in category_names, (
            f"Категория 'Относительные' не найдена в ответе. "
            f"Доступные категории: {category_names}"
        )
    ...
    def test_categories_filter_no_results(self, api_client):
        ...
        data = response.json()
        assert len(data) == 0, (
            f"Ожидался пустой список для несуществующей категории 99999, "
            f"получено {len(data)} записей. Данные: {data}"
        )

    def test_detail_contraindication(self, api_client,
                                     sample_contraindications):
        """Получение детальной информации о противопоказании"""
        contraindication = sample_contraindications[0]
        url = reverse('contraindication-detail',
                      kwargs={'pk': contraindication.id})
        response = api_client.get(url)

        assert response.status_code == 200, (
            f"Ожидался статус 200, получен {response.status_code}. "
            f"URL: {url}, pk={contraindication.id}"
        )
        data = response.json()
        assert data['name'] == contraindication.name, (
            f"Ожидалось имя '{contraindication.name}', "
            f"получено '{data['name']}'. Данные: {data}"
        )

        category_ids = [c['id'] for c in data['categories']]
        db_category_ids = list(
            contraindication.categories.values_list('id', flat=True))
    ...
    def test_contraindication_not_found(self, api_client):
        ...
        assert response.status_code == 404, (
            f"Ожидался статус 404 для несуществующего противопоказания, "
            f"получен {response.status_code}. URL: {url}"
        )

    def test_search_contraindications(self, api_client,
                                      sample_contraindications):
        ...
        data = response.json()
        assert len(data) > 0, (
            f"Ожидался хотя бы 1 результат поиска по 'Аллергия', "
            f"получено {len(data)} результатов. Данные: {data}"
        )
        assert 'Аллергия' in data[0]['name'], (
            f"Ожидалось 'Аллергия' в имени первого результата, "
            f"получено '{data[0]['name']}'. Данные: {data[0]}"
        )
    ...
    def test_search_contraindications_no_results(self, api_client):
        ...
        data = response.json()
        assert len(data) == 0, (
            f"Ожидался пустой список для поиска 'Несуществующее', "
            f"получено {len(data)} записей. Данные: {data}"
        )

    def test_filter_by_categories_name(self, api_client,
                                       sample_contraindications,
                                       sample_categories):
